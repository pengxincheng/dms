    ...
    <div class="main-content">
        <div class="main-content-inner fixed-page-header fixed-82">
            <div id="breadcrumbs" class="breadcrumbs">
                <ul class="breadcrumb">
                    <li class="active"><i class="fa fa-arrow-right"></i>修改宿舍区</li>
                </ul><!-- /.breadcrumb -->
        ...
        <div class="main-content-inner padding-page-content">
            <div class="page-content">
                ...
                <div class="row">
                    <div class=" col-xs-12">
                        <form class="form-horizontal" role="form" id="editCheckHygiene" name="editCheckHygiene" action="editCheckHygiene.do" method="post">
                            <div class="form-group">
                                <label class="col-sm-1 control-label no-padding-right" for="checkMan">检查人</label>
                                <div class="col-sm-3">
                                    <input type="text" data-validation-engine="validate[required]" class="form-control" placeholder="检查人" id="checkMan" name="checkMan" value="${checkHygiene.checkMan }"/>
                                </div>                                            
                            </div>
                            <div class="form-group">
                               <label class="col-sm-1 control-label no-padding-right" for="roomId">宿舍</label>
                                <div class="col-sm-3">
                                    <select class="form-control" id="roomId" name="roomId" data-validation-engine="validate[required]">
                                        <option value="" selected="selected">-请选择-</option>
                                        <c:forEach var="room" items="${rooms }">
                                        	<option ${checkHygiene.roomId eq room.roomId ? 'selected':''} value="${room.roomId}">${room.roomSn}</option>
                                        </c:forEach>
                                    </select>
                                </div>                                          
                            </div>
                            <div class="form-group">
                               <label class="col-sm-1 control-label no-padding-right" for="mark">评分</label>
                                <div class="col-sm-3">
                                    <select class="form-control" id="mark" name="mark" data-validation-engine="validate[required]">
                                        <option value="" selected="selected">-请选择-</option>
                                        <option ${checkHygiene.mark eq 5 ? 'selected':''} value="5">A</option>
                                        <option ${checkHygiene.mark eq 4 ? 'selected':''} value="4">B</option>
                                        <option ${checkHygiene.mark eq 3 ? 'selected':''} value="3">C</option>
                                        <option ${checkHygiene.mark eq 2 ? 'selected':''} value="2">D</option>
                                        <option ${checkHygiene.mark eq 1 ? 'selected':''} value="1">E</option>
                                    </select>
                                </div>                                          
                            </div>
							...
							<div class="form-group">
                                <label class="col-sm-1 control-label no-padding-right" for="describe">描述</label>
	                             <div class="col-sm-11">
	                                 <textarea data-validation-engine="validate[required]" class="form-control" id="describe" name="describe" placeholder="描述" style="width: 356px; height: 66px;">${checkHygiene.describe }</textarea>
	                             </div>
                            </div>
                            <input type="hidden" name="id" id="id" value="${checkHygiene.id }"/>					
                        </form>
...
<script type="text/javascript">
    jQuery(function ($) {
        ...
        $("#editCheckHygiene").validationEngine({
        ...
        $("#btnSave").on(ace.click_event, function () {
            //console.log("validate begin..");
            //console.log($('#addNotify').validationEngine('validate'));
            if ($('#editCheckHygiene').validationEngine('validate')) {
            	alert("操作成功");
                $('#editCheckHygiene').submit();
            }
        });
